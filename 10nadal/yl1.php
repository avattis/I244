...
<form method="POST" action="?">
    <textarea name="text" placeholder="kommentaari tekst"><?php if(!empty($_POST["text"]))
        echo htmlspecialchars($_POST["text"]); ?></textarea>
    <br/>
    <input type="color" name="bg" id="bg" <?php if(!empty($_POST["bg"]))
        echo "value=\"".htmlspecialchars($_POST["bg"])."\" "; ?> >
    <label for="bg">Taustavärvus</label>
    <br/>
    <input type="color" name="tc" id="tc" <?php if(!empty($_POST["tc"]))
        echo "value=\"".htmlspecialchars($_POST["tc"])."\" "; ?>>
    <label for="tc">Tekstivärvus</label>
    <br/>
    <fieldset>
        <legend>Piirjoon</legend>
        <input type="number" min="0" max="20" step="1" name="bw" id="bw" <?php if(!empty($_POST["bw"])
            && is_numeric($_POST["bw"]))
            echo "value=\"".htmlspecialchars($_POST["bw"])."\" "; ?> >
        ...
        <select name="bs">
            ...
            <?php endforeach; ?>
            <?php if(!empty($_POST["bs"]))
                echo "value=\"".htmlspecialchars($_POST["bs"])."\" "; ?>
        </select>
        <br/>
        <input type="color" name="bc" id="bc" <?php if(!empty($_POST["bc"]))
            echo "value=\"".htmlspecialchars($_POST["bc"])."\" "; ?>>
        <label for="bc">Piirjoone värvus</label>
        <br/>
        <input type="number" min="0" max="100" step="1" name="br" id="br" <?php if(!empty($_POST["br"])
            && is_numeric($_POST["br"]))
            echo "value=\"".htmlspecialchars($_POST["br"])."\" "; ?>>
